...
if ($_POST[uploading] == "yes") {
        move_uploaded_file($_FILES[download][tmp_name],DOWNLOADS_DIR.$_FILES[download][name]) or die("Couldn't move file");
...
if ($_GET[del]) {
        ...
        while ($row = mysql_fetch_array($result)) {
                unlink(DOWNLOADS_DIR.$row[name]); // or die(DOWNLOADS_DIR.$row[name]." -- Couldn't delete file");
        }
...
while ($row = mysql_fetch_array($result)) {
        ...
        if ($user->getRank() == "admin") {
                $wall_block .= "<a href=\"".SITE_DIR."downloads.php?del=$row[id]\" title=\"Delete this wallpaper?\">del</a>";
        }
        $count++;
}
...
while ($row = mysql_fetch_array($result)) {
        ...
        $file_size = stat(DOWNLOADS_DIR.$row[name]);
        $size = number_format($file_size[7] / 1024, 2, '.', ',');

        if ($user->getRank() == "admin") {
                $delete_link = "--[<a href=\"".SITE_DIR."downloads.php?del=$row[id]\" title=\"Delete this download?\">del</a>]";
        }
        $game_block .= "
        \t<tr>
...
while ($row = mysql_fetch_array($result)) {
        ...
        $size = number_format(filesize(DOWNLOADS_DIR.$row[name]) / 1024, 2, '.', ',');
        if ($user->getRank() == "admin") {
...
$header = "";
pageHeader("news", "squad", "Downloads", $header);
?>
<div align="center">
<center>
